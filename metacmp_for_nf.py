...
if __name__ == '__main__':
    ...
    if '-h' in myargs or len(myargs) == 0:  # Example usage.
        print('\nUsage: ./metacmp.py -c filename1.fa -g filename2.fa -o output [-t 8] \n')
        print('\t-c: Specify FASTA file containing assembled contigs (e.g. resulted from IDBA-UD).')
        print('\t-g: Specify FASTA file containing predicted genes (derived from prodigal).')
        print('\t-t: Specify the number of threads will be used in executing blast (default: 8).')
        print('\t-v: Printing important values in calculation On/Off [1: On (default), 0: Off]')
        print('\t-o: Output file that contains all stats and risk scores')
        print()
        exit()
    ...
    #sample_name = os.path.basename(myargs['-c']).split('.')[0]
    sample_name = myargs['-o']
    ...
    if not os.path.getsize(os.getcwd()+"/"+ card_name) > 0:
        ...
        f = open(myargs['-o'] + "_resistome_risk.txt", "a")
        f.write("Resistome Risk Score --> " + "0" + "\n") # no ARG found
        ...
        if myargs['-v'] == '1':
            ...
            f.write("Resistome Risk Score --> " + str(score) + "\n")
            
            f.write("nContigs --> " + str(nContigs) + "\n")
            f.write("nARG --> " + str(nARG) + "\n")
            f.write("nMGE --> " + str(nMGE) + "\n")
            f.write("nPAT --> " + str(nPAT) + "\n")
            f.write("nARG&MGE --> " + str(nARG_MGE) + "\n")
            f.write("nARG&MGE&PAT --> " + str(nARG_MGE_PAT) + "\n")
            f.write("nARG/nContigs --> " + str(fARG) + "\n")
            f.write("nMGE/nContigs --> " + str(fMGE) + "\n")
            f.write("nPAT/nContigs --> " + str(fPAT) + "\n")
            f.write("nARG&MGE/nContigs --> " + str(fARG_MGE) + "\n")
            f.write("nARG&MGE&PAT/nContigs --> " + str(fARG_MGE_PAT) + "\n")
